...
import httpx # requests 대신 httpx로 변경
...
async def to_thread(fn, *args, **kwargs):
    ...
    This safely passes keyword arguments to the function being run in the executor.
    """
...
async def romanize(text: str = Form(...)):
    # 동기 함수를 비동기적으로 실행하기 위해 to_thread 사용
...
async def translate_to_easy_korean(input_data: TextInput):
    ...
    try:
        ...
        # to_thread를 통해 동기 함수인 create_chat_completion_sync를 비동기적으로 호출
        translated_text = await to_thread(
            create_chat_completion_sync, SYSTEM_PROMPT, input_data.text, model="gpt-4o-mini", temperature=0.7
        )
        ...
        # asyncio.gather를 사용하여 여러 비동기 작업을 동시에 처리
        original_romanized_pronunciation_task = to_thread(convert_pronunciation_to_roman_sync, input_data.text)
        translated_romanized_pronunciation_task = to_thread(convert_pronunciation_to_roman_sync, translated_text)
        translated_english_translation_task = translate_korean_to_english_async(translated_text)
        ...
        keywords_okt = await asyncio.gather(
            original_romanized_pronunciation_task,
            translated_romanized_pronunciation_task,
            translated_english_translation_task,
            keywords_okt_task
        )
